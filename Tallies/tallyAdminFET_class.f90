module tallyAdminFET_class
  ...
  !! }
  !!
  type, public :: tallyAdminFET
    private
    ! Attachment
    type(tallyAdminFET),pointer :: atch => null()  ! Pointer to tallyAdmin attachment
...
contains
  ...
  subroutine init(self,dict)
    class(tallyAdminFET), intent(inout)            :: self
  ...
  recursive subroutine push(self, atch)
    class(tallyAdminFET), intent(inout)      :: self
    type(tallyAdminFET), pointer, intent(in) :: atch
  ...
  recursive subroutine pop(self, atch)
    class(tallyAdminFET), intent(inout)       :: self
    type(tallyAdminFET), pointer, intent(out) :: atch
  ...
  recursive function getEnd(self) result(atch)
    class(tallyAdminFET), intent(in)    :: self
    type(tallyAdminFET),pointer         :: atch
...
end module tallyAdminFET_class